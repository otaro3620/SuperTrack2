...
public class VehicleControl : MonoBehaviour
{
    ...
    public void ShiftUp()
    {
        float now = Time.timeSinceLevelLoad;//tiempo que ha transcurrido desde que la ultima esena se cargo 

        if (now < shiftDelay/*retraso de turno*/) return;
    ...
    void FixedUpdate()
    {
        ...
        if (activeControl)
        {
            ...
            if (controlMode == ControlMode.simple)
            {
                ...
                {
                    steer = Mathf.MoveTowards(steer, Input.GetAxis("Horizontalplayer1"), 0.2f);
                    accel = Input.GetAxis("Verticalplayer1");
    ...
    : slip = Mathf.Lerp(slip, 0.01f, 0.02f);
            ...
            if (shift && (currentGear > 1 && speed > 50.0f) && shifmotor && Mathf.Abs(steer) < 0.2f)
            {
                ...
                var emision1 = carParticles.shiftParticle1.emission;
                var emision2 = carParticles.shiftParticle2.emission;
                emision1.rateOverTime = Mathf.Lerp(carParticles.shiftParticle1.emission.rateOverTimeMultiplier, powerShift > 0 ? 50 : 0, Time.deltaTime * 10.0f);
                emision2.rateOverTime = Mathf.Lerp(carParticles.shiftParticle2.emission.rateOverTimeMultiplier, powerShift > 0 ? 50 : 0, Time.deltaTime * 10.0f);
            }
            else
            {
                ...
                var emision3 = carParticles.shiftParticle1.emission;
                var emision4 = carParticles.shiftParticle2.emission;
                emision3.rateOverTime = Mathf.Lerp(carParticles.shiftParticle1.emission.rateOverTimeMultiplier, 0, Time.deltaTime * 10.0f);
                emision4.rateOverTime = Mathf.Lerp(carParticles.shiftParticle2.emission.rateOverTimeMultiplier, 0, Time.deltaTime * 10.0f);
            }
                ...
                if (carParticles.brakeParticlePerfab)
                {
                    ...
                    else if ((brake || Mathf.Abs(hit.sidewaysSlip) > 0.6f) && speed > 1)
                    {
                        ...
                        if ((accel < 0.0f) || ((brake || Mathf.Abs(hit.sidewaysSlip) > 0.6f) && (w == wheels[2] || w == wheels[3])))
                        {
                            ...
                            if (!Particle[currentWheel].GetComponent<AudioSource>().isPlaying)
                                Particle[currentWheel].GetComponent<AudioSource>().Play();
                            var emision1 = pc.emission;
                            emision1.enabled = true;
                    ...
                    {
                        var emision3 = pc.emission;
                        emision3.enabled = false;
            ...
            else
            {
                ...
                {
                    var pc = Particle[currentWheel].GetComponent<ParticleSystem>();
                    var emision4 = pc.emission;
                    emision4.enabled = false;
                }
